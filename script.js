...
// Improved fetch with CORS handling and error management
async function fetchData(url, options = {}) {
    try {
        ...
        updateSyncStatus(`Network error: ${error.message}`, 'error');
        throw error;
    }
}
...
async function apiRequest(action, data = null) {
    ...
    try {
        ...
        updateSyncStatus('Operation successful', 'success');
        return response;
    } catch (error) {
...
function openRatingModal(id) {
    ...
    document.getElementById('date-watched').value = item.dateWatched ? dateUtils.toYMD(item.dateWatched) : '';
...
function saveRating() {
    ...
    const dateWatched = dateWatchedInput ? dateUtils.toDMY(dateWatchedInput) : '';
...
async function syncData() {
    ...
    try {
        const payload = {
            movies: watchlistData.filter(i => i.type === 'Movie').map(i => i.rawData),
            shows: watchlistData.filter(i => i.type === 'Show').map(i => i.rawData),
            seasons: watchlistData.filter(i => i.type === 'Season').map(i => i.rawData),
